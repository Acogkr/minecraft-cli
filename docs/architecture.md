...
실제 GUI, Lore, 채팅 hover, 타이틀, 액션바, 보스바, 스코어보드, 토스트와 다이얼로그를 렌더링합니다. 좌표 클릭, 가상 커서, 텍스트 입력, 키보드 탐색, 스크롤은 Minecraft 내부 이벤트로 전달되어 사용자의 Windows 포커스와 입력 장치를 사용하지 않습니다. 각 시각 세션은 별도 제어 포트와 무작위 토큰을 사용합니다.
